...
| `SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_USER`, `SMTP_PASS`, `MAIL_FROM` | your mail provider. **`SMTP_HOST` is required in production** — without it order/reservation emails silently wouldn't be sent, so the server refuses to boot. To launch before you have a mail provider, set `ALLOW_NO_SMTP=true`: the server then boots with a loud startup warning, and emails are only logged (masked recipient + subject), never sent — customers get no confirmations and you get no alerts, so watch the admin dashboard. Remove it once SMTP is configured. `MAIL_FROM` should be an address on a domain you've set up SPF/DKIM for. |
